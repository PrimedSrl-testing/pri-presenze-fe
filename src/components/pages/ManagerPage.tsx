// src/components/pages/ManagerPage.tsx
'use client';
...
export default function ManagerPage() {
  ...
  const [activeTab, setActiveTab] = useState<'team' | 'approvals' | 'presenze'>('team');
  const [requests, setRequests] = useState<PendingRequest[]>(PENDING_REQUESTS);

  const myTeam = EMPLOYEES.filter((e) => e.dept === user?.dept && e.active && e.role !== 'mgr');

  const handleApprove = (id: string) => {
    setRequests((prev) => prev.map((r) => r.id === id ? { ...r, stato: 'approvata' as const } : r));
    showToast('Richiesta approvata', 'ok');
  };

  const handleReject = (id: string) => {
    setRequests((prev) => prev.map((r) => r.id === id ? { ...r, stato: 'rifiutata' as const } : r));
        ...
        <div className="tab-content">
          <div className="table-wrapper">
            <table className="data-table">
              <thead>
                <tr>
                  <th>Dipendente</th><th>Mat.</th><th>Contratto</th><th>Ore/sett</th><th>Stato</th>
                </tr>
              </thead>
              <tbody>
                {myTeam.map((emp) => (
                ...
                {myTeam.length === 0 && (
                  <tr><td colSpan={5} className="table-empty">Nessun dipendente trovato per questo reparto.</td></tr>
                )}
              </tbody>
            </table>
          </div>
              ...
              {requests.map((req) => (
                <div key={req.id} className="request-card">
                  <div className="request-card-header">
                    ...
                    <span className="badge" style={{ background: STATO_COLORS[req.stato] + '20', color: STATO_COLORS[req.stato] }}>
                      {req.stato.replace('_', ' ')}
                    </span>
                  </div>
                  ...
                  {req.stato === 'in_attesa' && (
                    <div className="request-actions">
                      <button className="btn btn--success btn--sm" onClick={() => handleApprove(req.id)}>✓ Approva</button>
                      <button className="btn btn--danger btn--sm"  onClick={() => handleReject(req.id)}>✕ Rifiuta</button>
                    </div>
                  )}
                </div>
              ))}
      ...
      {activeTab === 'presenze' && (
        <div className="tab-content">
          <div className="table-wrapper">
            <table className="data-table">
              <thead>
                <tr><th>Dipendente</th><th>Ore timbrate</th><th>Ore contratto</th><th>Δ</th><th>Assenze</th></tr>
              </thead>
              <tbody>
                {myTeam.map((emp) => {
                  const delta = 2.3;
                  return (
                    <tr key={emp.id}>
                      ...
                      <td><span className="mono" style={{ color: delta >= 0 ? 'var(--green)' : 'var(--red)' }}>{delta >= 0 ? '+' : ''}{delta}h</span></td>
                      <td>0gg</td>
                    </tr>
                  );
                })}
