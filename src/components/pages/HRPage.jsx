// client/src/components/pages/HRPage.jsx
// ─────────────────────────────────────────────────────────────
// Vista HR: gestione dipendenti, approvazioni, report, config
// ─────────────────────────────────────────────────────────────
...
export default function HRPage() {
  ...
  const filtered = EMPLOYEES.filter(emp => {
    const matchSearch = !search ||
      emp.full.toLowerCase().includes(search.toLowerCase()) ||
      emp.mat.includes(search);
    const matchDept = !deptFilter || emp.dept === deptFilter;
    return matchSearch && matchDept;
  });

  const scadenzeProssime = EMPLOYEES.filter(e =>
    e.fin_contratto && new Date(e.fin_contratto) < new Date('2026-06-01')
  );
    ...
    <div className="page page--hr">
      ...
      <div className="hr-subnav">
        {HR_SECTIONS.map(s => (
          <button
            key={s.id}
            className={`hr-subnav-btn ${section === s.id ? 'active' : ''}`}
                ...
                <tbody>
                  {allDepts.map(dept => {
                    ...
                    return (
                      <tr key={dept}>
                        <td>
                          <span className="dept-badge" style={{ background: (cfg?.col || '#6b7280') + '20', color: cfg?.col || '#6b7280' }}>
                            {dept}
                          </span>
                        </td>
                        ...
                        <td>
                          {temp.length > 0
                            ? <span className="badge badge--warning">{temp.length}</span>
                    ...
                    <tr key={emp.id}>
                      <td>
                        <div className="emp-cell">
                          <div className="emp-avatar-sm"
                            style={{ background: emp.col + '22', color: emp.col }}>
                            {emp.ini}
                          </div>
                          <div>
                      ...
                      <td>
                        <span className="dept-badge"
                          style={{ background: (dept?.col || '#6b7280') + '20', color: dept?.col || '#6b7280' }}>
                          {emp.dept}
                        </span>
                      </td>
                      <td>{emp.contract}</td>
                      ...
                      <td>
                        <span className={`badge badge--${emp.active ? 'success' : 'neutral'}`}>
                          {emp.active ? 'Attivo' : 'Non attivo'}
                        </span>
                      </td>
                      <td>
                        {emp.fin_contratto
                          ? <span className="mono badge badge--warning">{emp.fin_contratto}</span>
                          : <span className="mono" style={{ color: 'var(--muted)' }}>Indeterminato</span>}
                      </td>
                    </tr>
                  );
                })}
            ...
            {[
              ...
              <div key={r.title} className="report-card">
                ...
                <button
                  ...
                  onClick={() => showToast(`${r.title} — funzione disponibile in produzione`, 'info')}
                >
                  Genera
                </button>
              </div>
        ...
        <div className="tab-content">
          <div className="empty-state">
            ...
            <div className="empty-state-sub">
              Turni, causali, permessi ruolo, blocco mesi e dispositivi di timbratura.
            </div>
          </div>
        </div>
      )}
