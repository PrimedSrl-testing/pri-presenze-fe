// src/components/pages/DipendentePage.tsx
'use client';
...
export default function DipendentePage() {
  ...
  const [activeTab, setActiveTab] = useState<'riepilogo' | 'foglio' | 'richieste'>('riepilogo');
      ...
      {activeTab === 'riepilogo' && (
        <div className="tab-content">
          <div className="kpi-grid">
            <KpiCard label="Ore timbrate" value={`${d.ore_timb}h`} sub={`su ${d.ore_cons}h contratto`} color="var(--blue)" />
            <KpiCard label="Saldo ore" value={`${d.saldo_timb > 0 ? '+' : ''}${d.saldo_timb}h`} color={d.saldo_timb >= 0 ? 'var(--green)' : 'var(--red)'} />
            <KpiCard label="Ferie residue" value={`${d.fin_ferie > 0 ? '+' : ''}${d.fin_ferie}gg`} color={d.fin_ferie >= 0 ? 'var(--green)' : 'var(--orange)'} />
            <KpiCard label="ROL residuo" value={`${d.fin_rol > 0 ? '+' : ''}${d.fin_rol}h`} color={d.fin_rol >= 0 ? 'var(--green)' : 'var(--orange)'} />
          </div>
        </div>
      )}
      ...
      {activeTab === 'foglio' && (
        <div className="tab-content">
          <div className="timesheet-grid">
            {WEEK_DAYS.map((wd) => (
              <div key={wd} className="timesheet-header-cell">{wd}</div>
            ))}
            {MOCK_MONTH_DATA.daily.slice(1).map((ore, idx) => {
              const day = idx + 1;
              const isWeekend = ((day + 6) % 7) >= 5;
              const isEmpty = ore === 0;
              return (
                <div key={day} className={`timesheet-day ${isEmpty ? 'empty' : ''} ${isWeekend ? 'weekend' : ''}`}>
